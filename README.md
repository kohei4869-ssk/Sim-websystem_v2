...
これらを解消するため、依頼内容を構造化Girdフォームで入力＆仕様に基づいて条件ロジックで制御することで、正確かつ直感的に依頼が可能に。
担当部署への自動振り分け・対応状況の一元管理・見積もりデータの蓄積までを行う仕組みとして、SIM FORMを企画/開発しました。

## システム概要

依頼者は、Gird形式の入力フォーム（SimForm.vue）を使って、クライアント名・案件名・媒体・配信条件などを直感的に入力できます。
...
|---|---|---|
| PHP | Laravel | ルーティング・コントローラー・DB操作・画面生成など、サーバーサイド全体の処理 |
| JavaScript | Vue.js | 依頼入力フォーム（SimForm.vue）のUI構築・入力状態の管理 |
| JavaScript | jQuery | 管理画面（department.blade.php）でのDOM操作・イベント処理・Ajax通信 |
| JavaScript | Chart.js | 管理画面のグラフ描画（月別推移・ステータス内訳など） |
| SQL | MySQL | 依頼・案件・見積もりデータの永続化 |
| JavaScript（旧構成） | GAS（Google Apps Script） | 旧バージョンのバックエンド処理（フォーム送信の受付・Notion APIへのデータ送信・Slack通知）を担当。自分で構築・運用 |
| （旧構成） | Notion API | 依頼・案件データの保存先（親DB・子DBの2つに分けて登録） |

その他：Slack Incoming Webhookによる担当部署への自動通知
